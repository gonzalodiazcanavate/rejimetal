(function () {
  ...
  const lerp = function (a, b, t) { return point(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t); };
  const translate = function (p, dx, dy) { return point(p.x + dx, p.y + dy); };
  const polygonPoints = function (points) {
    return points.map(function (p) { return p.x.toFixed(2) + ',' + p.y.toFixed(2); }).join(' ');
  };
  const line = function (a, b, className) {
    return '<line class="' + className + '" x1="' + a.x.toFixed(2) + '" y1="' + a.y.toFixed(2) + '" x2="' + b.x.toFixed(2) + '" y2="' + b.y.toFixed(2) + '" />';
  };
  const path = function (d, className) { return '<path class="' + className + '" d="' + d + '" />'; };
  const polygon = function (points, className) { return '<polygon class="' + className + '" points="' + polygonPoints(points) + '" />'; };

  const isType = function (visualType, token) { return String(visualType || '').indexOf(token) !== -1; };
  const signatureFor = function (item, visualType) {
    return [visualType, item && item.id, item && item.name].filter(Boolean).join(' ').toLowerCase();
  };
  const has = function (signature, token) { return String(signature || '').indexOf(token) !== -1; };

  const shapeFor = function (visualType) {
  ...
  const gridCount = function (visualType, signature) {
  ...
  const renderBearingFlats = function (corners, count, className) {
    const A = corners[0]; const B = corners[1]; const C = corners[2]; const D = corners[3];
    let output = '';
  ...
  const renderCrossMembers = function (corners, count, className) {
    const A = corners[0]; const B = corners[1]; const C = corners[2]; const D = corners[3];
    let output = '';
    for (let i = 0; i < count; i += 1) {
  ...
  const renderElectroweldedGrid = function (corners, visualType, signature) {
  ...
  const renderPressedGrid = function (corners, visualType, signature) {
    const counts = gridCount(visualType, signature);
  ...
  const renderOffshoreLayer = function (corners) {
    const A = corners[0]; const B = corners[1]; const C = corners[2]; const D = corners[3];
  ...
  const renderGrid = function (corners, visualType, signature) {
    if (isType(visualType, 'pressed')) return renderPressedGrid(corners, visualType, signature);
    return renderElectroweldedGrid(corners, visualType, signature) + (isType(visualType, 'offshore') ? renderOffshoreLayer(corners) : '');
  };
  ...
  function renderPressedKeys(corners, bars, crosses) {
    ...
    for (let i = 2; i < bars - 2; i += 3) {
      ...
      for (let j = 2; j < crosses - 2; j += 4) {
        ...
        output += '<rect class="visual-pressed-key" x="' + (p.x - 1.8).toFixed(2) + '" y="' + (p.y - 1).toFixed(2) + '" width="3.6" height="2" rx="0.25" />';
      }
    }
    return output;
  }
  ...
  const renderProductVisual = function (item, options) {
    const opts = options || {};
    ...
    else if (isType(visualType, 'profile')) detail += renderProfileLip(corners);
  ...
  const renderWorkVisual = function (work, options) {
    const opts = options || {};
  ...
  window.REJIMETAL_renderProductVisual = renderProductVisual;
  window.REJIMETAL_renderWorkVisual = renderWorkVisual;
})();
